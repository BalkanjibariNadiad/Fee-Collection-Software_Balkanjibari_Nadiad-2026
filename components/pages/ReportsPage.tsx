...
export default function ReportsPage({ userRole }: ReportsPageProps) {
  ...
  const [dateWiseStartDate, setDateWiseStartDate] = useState(new Date().toISOString().split('T')[0])
  const [dateWiseEndDate, setDateWiseEndDate] = useState(new Date().toISOString().split('T')[0])
  const [dateWisePaymentMode, setDateWisePaymentMode] = useState<'ALL' | 'OFFLINE' | 'ONLINE'>('ALL')
  const [dateWiseLoading, setDateWiseLoading] = useState(false)
  const [dateWiseData, setDateWiseData] = useState<any | null>(null)
  ...
  const generateDateWiseFeeReport = async () => {
    ...
    try {
      ...
      const response = await analyticsApi.getDateWiseFeeReport(
        dateWiseStartDate,
        dateWiseEndDate,
        dateWisePaymentMode
      )
      const data = (response as any)?.data || response
      setDateWiseData(data || null)
      notifySuccess('Date-wise fee report generated successfully')
    } catch (error) {
      console.error('Date-wise fee report generation failed:', error)
      notifyError('Failed to generate date-wise fee report')
    } finally {
  ...
  const handleDateWiseDownload = async (format: 'CSV' | 'PDF') => {
    ...
    try {
      ...
      if (format === 'PDF') {
        await analyticsApi.exportDateWiseFeeReportPdf(dateWiseStartDate, dateWiseEndDate, dateWisePaymentMode)
      } else {
        await analyticsApi.exportDateWiseFeeReportCsv(dateWiseStartDate, dateWiseEndDate, dateWisePaymentMode)
      }
      notifySuccess(`${format} Report downloaded successfully`)
    } catch (error) {
  ...
  return (
    <div className="p-2.5 sm:p-6 space-y-4">
      ...
      <div className="bg-white p-4 sm:p-6 rounded-2xl border border-slate-100 shadow-sm">
        <div className="mb-4">
          <h2 className="text-lg sm:text-xl font-bold text-slate-900 uppercase tracking-tight font-poppins">Date-wise Fee Collection Report</h2>
          <p className="text-slate-500 text-sm mt-1 font-medium font-inter">Generate day-wise collection summary for a custom date range with payment mode filter.</p>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-4">
          <div>
            ...
            <input
              ...
              onChange={(event) => setDateWiseStartDate(event.target.value)}
              className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-900"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(event) => setDateWiseEndDate(event.target.value)}
              className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-900"
            />
          </div>
          ...
          <div>
            ...
            <select
              value={dateWisePaymentMode}
              onChange={(event) => setDateWisePaymentMode(event.target.value as 'ALL' | 'OFFLINE' | 'ONLINE')}
              className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm font-medium text-slate-900"
            >
          ...
          <div className="flex items-end">
            ...
            >
              {dateWiseLoading ? <Loader2 size={16} className="animate-spin" /> : <Calendar size={16} />}
              Generate Report
            </button>
          </div>
        </div>
        ...
        {dateWiseLoading ? (
          <div className="mt-4 rounded-2xl border border-slate-100 bg-white p-8 text-center text-slate-500 shadow-sm">
            <Loader2 size={18} className="mx-auto mb-3 animate-spin" />
            <p className="text-sm font-semibold uppercase tracking-widest">Loading date-wise report...</p>
          </div>
        ) : dateWiseData ? (
          <div className="mt-4 overflow-x-auto rounded-2xl border border-slate-100 bg-white shadow-sm">
            <table className="min-w-full divide-y divide-slate-200 text-xs">
              <thead className="bg-slate-900 text-white sticky top-0">
                <tr>
                  <th className="px-3 py-2 text-left text-[9px] font-bold uppercase tracking-widest">Sr. No.</th>
                  <th className="px-3 py-2 text-left text-[9px] font-bold uppercase tracking-widest">Date</th>
                  <th className="px-3 py-2 text-right text-[9px] font-bold uppercase tracking-widest">Offline Fees</th>
                  <th className="px-3 py-2 text-right text-[9px] font-bold uppercase tracking-widest">Online Fees</th>
                  <th className="px-3 py-2 text-right text-[9px] font-bold uppercase tracking-widest">Total Fees</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {dateWiseData.rows?.length > 0 ? (
                  dateWiseData.rows.map((row: any, index: number) => (
                    <tr key={`${row.date}-${index}`} className="bg-white hover:bg-slate-50 transition">
                      <td className="px-3 py-2 font-semibold text-slate-700">{row.sr_no || index + 1}</td>
                      <td className="px-3 py-2 text-slate-700">{row.date}</td>
                      <td className="px-3 py-2 text-right font-medium text-slate-900">{formatCurrency(row.offline_fees || 0)}</td>
                      <td className="px-3 py-2 text-right font-medium text-slate-900">{formatCurrency(row.online_fees || 0)}</td>
                      <td className="px-3 py-2 text-right font-bold text-emerald-700">{formatCurrency(row.total_fees || 0)}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
              ...
              <tfoot>
                <tr className="bg-slate-50 border-t border-slate-200">
                  <td colSpan={4} className="px-3 py-3 text-right text-[10px] font-bold uppercase tracking-widest text-slate-600">Grand Total</td>
                  <td className="px-3 py-3 text-right text-sm font-bold text-slate-900">{formatCurrency(dateWiseData.grand_total || 0)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
          ...
          <div className="mt-4 rounded-2xl border border-dashed border-slate-200 bg-white p-8 text-center text-slate-400">
            <p className="text-sm font-semibold uppercase tracking-widest">No date-wise report generated yet</p>
            <p className="mt-2 text-xs">Select start date, end date, and payment mode, then click Generate Report.</p>
          </div>
        )}
      </div>
    </div>
